...
def cupy_kernel(strFunction, objectVariables):
	...
	while True:
		...
		intArg = int(objectMatch[2])

		strTensor = objectMatch[4]
	...
	while True:
		...
		intArgs = int(objectMatch[2])
		strArgs = objectMatch[4].split(',')
		...
		strKernel = strKernel.replace(
			objectMatch[0], f'{strTensor}[' + str.join('+', strIndex) + ']'
		)
	...
	def forward(self, first, second):
		...
		if first.is_cuda == True:
			...
			cupy_launch(
				...
				grid=(int((n + 16 - 1) / 16), first.size(1), first.size(0)),
				block=(16, 1, 1),
			...
			cupy_launch(
				...
				grid=(int((n + 16 - 1) / 16), second.size(1), second.size(0)),
				block=(16, 1, 1),
			...
			cupy_launch(
				...
				grid=(output.size(3), output.size(2), output.size(0)),
				block=(32, 1, 1),
